...
export const addDoctorSubscription = async (
  subscription: DoctorSubscriptionsInterface,
) => {
  return db
    .insert(schema_management.doctorSubcriptions)
    .values(subscription)
    .onConflictDoNothing()
    .returning();
};
...
export const setDoctorSubscription = async (
  ...
  return db
    .update(schema_management.doctorSubcriptions)
    .set(subscription)
...
export const getDoctorSubscription = async (
  ...
  if (stripe_customer_id) {
    return db.query.doctorSubcriptions.findFirst({
      where: eq(
        schema_management.doctorSubcriptions.stripeCustomerId,
        stripe_customer_id,
      ),
    });
  }
